...
- Must match the H1 in intent; they should be similar but not necessarily identical
- Add `(Exclusive)` or `(Excl)` at the end when your outlet is first to publish
...
- When new information is added to an article
- When better keywords are identified via Google Trends
- When the SEO title needs to be made clearer or more search-aligned
- **Note:** Articles are indexed almost immediately after publishing. Optimize before hitting publish.

## 1.3 Explicit Language Policy

The following will suppress articles from Google News, Google Discover, and syndication partner feeds. Continuous violations may result in manual actions against the site.
...
- Restricted language must be avoided in all gallery body copy, headlines, SEO titles, promo titles, URLs, and meta descriptions at all times
...
<!-- FORMAT:additional-formats-pending -->
## 3.2 Additional Formats — Pending

> ⚠️ **STUB** — Standards for additional article formats are pending. Do not create or publish content under a new format type until its section has been reviewed and approved. Add new formats following the §3.1 structure: Purpose, required fields, article structure, formatting rules, pre-publish checklist.
